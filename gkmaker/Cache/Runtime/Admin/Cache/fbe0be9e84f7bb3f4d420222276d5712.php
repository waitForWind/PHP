...
<div class="clear"></div>
    ...
    <div class="QRcode">移动设备访问本页：<br/><br/><img src="<?php echo ($QRcodeUrl); ?>"/></div>
        ...
        <div id="Right">
            <div class="Item hr">
                <div class="current">编辑教师</div>
            </div>
            <form action="__URL__/editTeacher" method="post" enctype="multipart/form-data">




                <input type="hidden" id="num" name="num" value=<?php echo ($num); ?> />
                <input type="hidden" name="info[uid]" value=<?php echo ($id); ?> />
                <table width="480" border="0">
                    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$in): $mod = ($i % 2 );++$i;?><tr>
                        <td align="right">教师<?php echo ($i); ?>姓名:<input type="text" name="info[teacher<?php echo ($i); ?>]" value=<?php echo ($info[$i]); ?>></td>

                    </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                    <tbody id="team_apply"></tbody>
                    <tr>

                        <td align="left" height="100"><input type="button" id="addmore" value="增加教师+" name="addmore" onclick="add()"></td>
                    </tr>
...
<script type="text/javascript">
    ...
    function add(){
        ...
        var html='<tr>';
        html+='<tr>';
        html+='<td align="center">教师'+num+'姓名:</td><td align="center">';
        html+='<select name="info[teacher'+num+']">';
        html+='<option selected="selected" value="">请选择</option>';
        html+='<?php if(is_array($info1)): $i = 0; $__LIST__ = $info1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>';
