                ...
                <div id="Right">
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center" id="<?php echo ($vo["id"]); ?>">
                                <td width="200px"><a style="color:red" href="__URL__/dowfile?id=<?php echo ($vo["id"]); ?>"><?php echo ($vo["title"]); ?></a></td>
                                <td><?php if($vo["status"] == 2): ?>待审核<?php else: ?>已通过<?php endif; ?></td>
                                <td><?php echo (date("Y-m-d H:i:s",$vo["time"])); ?></td>
                                <!-- <td><?php echo ($vo["aidName"]); ?></td> -->
                                <td width="300px">   [ <a href="__URL__/status?id=<?php echo ($vo["id"]); ?>">
                                    <?php if($vo["status"] == 1 ): ?>申请失败
                                        <?php else: ?>申请通过<?php endif; ?></a> ][ <a href="__URL__/edit?id=<?php echo ($vo["id"]); ?>">添加教师 </a> ] [ <a link="<?php echo U('Science/delfile/',array('id'=>$vo['id']));?>" href="javascript:void(0)" name="<?php echo ($vo["title"]); ?>" class="del">删除 </a> ]</td>
