...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>科技立项-<?php echo ($site["SITE_INFO"]["name"]); ?></title>
        <?php $addCss=""; $addJs=""; $currentNav ='科技立项 > 科技立项简介'; ?>
...
<div class="clear"></div>
    ...
    <div class="QRcode">移动设备访问本页：<br/><br/><img src="<?php echo ($QRcodeUrl); ?>"/></div>
                ...
                <div id="Right">  
				<div class="Item hr">
                        <div class="current">添加简介</div>
                    </div>
                        <input type="button" name="toadd" value="添加简介" class="btn" onclick="window.location.href='__URL__/addIntroduction'"/>
					
                    <div class="Item hr">
                        <div class="current">简介列表</div>
                    </div>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center" id="<?php echo ($vo["id"]); ?>">
                                ...
                                <td><?php if($vo["status"]): ?>已发布状态<?php else: ?>审核状态<?php endif; ?></td>
                                <td><?php echo (date("Y-m-d H:i:s",$vo["published"])); ?></td>                       
                                <td>[<a href="__URL__/putout3?id=<?php echo ($vo["id"]); ?>"><?php if($vo["status"]): ?>取消发布<?php else: ?>点击发布<?php endif; ?></a>][ <a href="__URL__/addIntroduction?id=<?php echo ($vo["id"]); ?>">编辑 </a> ] [ <a link="<?php echo U('Science/delIntroduction/',array('id'=>$vo['id']));?>"  name="<?php echo ($vo["title"]); ?>" class="del">删除 </a> ]</td>
