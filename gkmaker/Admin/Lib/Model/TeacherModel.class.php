...
class TeacherModel extends Model {
        ...
        if (IS_POST) {
            ...
            if ($act == "add") { //添加分类
                ...
                if ($M->where($data)->count() == 0) {
                    return ($M->add($data)) ? array('status' => 1, 'info' => '分类 ' . $data['name'] . ' 已经成功添加到系统中', 'url' => U('Teacher/category', array('time' => time()))) : array('status' => 0, 'info' => '分类 ' . $data['name'] . ' 添加失败');
                } else {
                ...
                return ($M->save($data)) ? array('status' => 1, 'info' => '分类 ' . $data['name'] . ' 已经成功更新', 'url' => U('Teacher/category', array('time' => time()))) : array('status' => 0, 'info' => '分类 ' . $data['name'] . ' 更新失败');
            } else if ($act == "del") { //删除分类
                unset($data['pid'], $data['name']);
                return ($M->where($data)->delete()) ? array('status' => 1, 'info' => '分类 ' . $data['name'] . ' 已经成功删除', 'url' => U('Teacher/category', array('time' => time()))) : array('status' => 0, 'info' => '分类 ' . $data['name'] . ' 删除失败');
            }
        } else {
            import("Category");
    ...
    public function addNews() {
        ...
        if ($M->add($data)) {
            return array('status' => 1, 'info' => "已经发布", 'url' => U('Teacher/index'));
        } else {
    ...
    public function edit() {
        ...
        if ($M->save($data)) {
            return array('status' => 1, 'info' => "已经更新", 'url' => U('Teacher/index'));
        } else {
